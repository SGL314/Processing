...
void draw() {
	...
	// }
	// adl1k.draw();
	modelosAtuais.get(modeloAtual).draw(); 
...
void mousePressed() {
	// botão direito 
	if (mouseButton == LEFT) {
		if (moises.firstPressed) {
			moises.points[0][0] = mouseX;
			moises.points[0][1] = mouseY;
			moises.firstPressed = false;
		} else{
			moises.points[1][0] = mouseX;
			moises.points[1][1] = mouseY;
			moises.firstPressed = true;
		}
	} else if (mouseButton == RIGHT) {
		moises.points = new float[][] { {0, 0} , {0, 0} };
	}
}
...
void keyReleased() {
	...
	if (modeler.ctrlPressed) {
	    ...
	    if (keyCode == LEFT) { // sizeDsOWN
	        modeloAtual -= 1;
	    } else if (keyCode == RIGHT) { // sizeUP
	        modeloAtual += 1;
	    }
	    modeloAtual = (modeloAtual>modelosAtuais.size() - 1) ? modeloAtual - 1 : modeloAtual;
	    modeloAtual = (modeloAtual<0) ? 0 : modeloAtual;
	}
