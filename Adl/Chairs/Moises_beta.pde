class Moises_beta extends Modelo{
	int longitude = 30,
	...
	paddingX = 6; // distancia da borda até o chão do Nobre
	...
	void painelDeLuz(){
		fill(#2493FF);
		float tx = 100,ty=10,
		px = width/2+longitude/2*tamQuadrado,
		py = espessuraAltar+(altura-5)*tamQuadrado;
		rect(px,py,tx,ty);
	}
	...
	void blackTiles(){
		...
		// textSize(size);
		// text(texto,px,py);
		// lado direitosuperBlackTile(width/2+(1+longitude/2)*tamQuadrado,espessuraAltar); // na entrada da escada direita
	...
	void paredes(){
		...
		//retas
		rect((width-1000)/2-3-paddingX,0,125,2*tamQuadrado);
		//tampa
		fill(#ffffff);
		...
		//retas
		rect((width-1000)/2+872-paddingX,0,width-872,2*tamQuadrado);
		//tampa
		fill(#ffffff);
	...
	void linhasImaginarias(){
        ...
        // meridiano
        stroke(#ff0000);
        line(width/2,espessuraAltar,width/2,espessuraAltar+altura*tamQuadrado+espessuraFaixa);
        fill(#ffffff);
        size = 20;
        texto = "Meio do Nobre";
        px = 510;
        ...
        text(texto,px,py);
        stroke(#AC21CB);
        line(width/2-longitudeAltar/2,espessuraAltar,width/2-longitudeAltar/2,espessuraAltar+altura*tamQuadrado+espessuraFaixa);
        fill(#ffffff);
        size = 20;
        ...
        // maxima pulpital
        stroke(#C4B102);
        line(width/2+tamQuadrado*2,espessuraAltar,width/2+tamQuadrado*2,espessuraAltar+distBlocos[0]*tamQuadrado);
        fill(#ffffff);
        size = 20;
        texto = ((int)distBlocos[0])+" blocos";
        px = 545;
        py = 125;
        noStroke();
        textSize(size);
        ...
        // bloco Meridional
        stroke(#2E60E5);
        line(width/2-tamQuadrado*espacoAteMeridianoCentral,espessuraAltar+(distBlocos[0])*tamQuadrado,width/2,espessuraAltar+(distBlocos[0])*tamQuadrado);
        fill(#ffffff);
        size = 20;
        texto = espacoAteMeridianoCentral+" blocos";
        px = 430;
        py = 125-6*tamQuadrado + distBlocos[0]*tamQuadrado;
        noStroke();
        textSize(size);
        rect(px,py-size+3,textWidth(texto),size);
        fill(#2E60E5);
    ...
    void linhasAuxiliares(){
        ...
        // definição de pivots
        centroX = width/2;
        centroY = espessuraAltar+distBlocos[0]*tamQuadrado;
        ...
        line(
            centroX-(longitude/2+2)*tamQuadrado , espessuraAltar+distBlocos[0]*tamQuadrado-tamQuadrado,
            centroX-(longitude/2+2)*tamQuadrado , espessuraAltar+distBlocos[0]*tamQuadrado+qtFileirasLaterais*tamQuadrado*4+tamQuadrado
        );
        line(
            centroX-(longitude/2+2)*tamQuadrado -varX, espessuraAltar+distBlocos[0]*tamQuadrado-tamQuadrado-varY-alignUp_001+(2)*tamQuadrado*4,
            centroX-(longitude/2+2)*tamQuadrado -varX, espessuraAltar+distBlocos[0]*tamQuadrado+qtFileirasLaterais*tamQuadrado*4+tamQuadrado
        );
	...
	void dados(){
        ...
        float py = 767;
        textSize(size);
        rect(px,py-size+3,textWidth(texto)+2,size);
        fill(#000000);
        text(texto,px,py);
		size = 15;
        ...
        // size = 20;
        texto = "Bebedouro";
        px  = 233+(width-1200)/2+textWidth(texto)/2+7;
        py = 890;
        textSize(size);
        rect(px,py-size+3,textWidth(texto),size);
        fill(#000000);
        ...
        texto = "Painel de Luz";
		px = width/2+longitude/2*tamQuadrado+100+4;
		py = espessuraAltar+(altura-5)*tamQuadrado+10;
        rect(px,py-size+3,textWidth(texto)+2,size);
        fill(#2493FF);
        textSize(size);
        text(texto,px,py);
	...
	void position(){
		...
		float t = textWidth(texto);
		text(texto,mouseX,mouseY);

		float vx = (firstPressed) ? points[1][0] : mouseX;
		float vy = (firstPressed) ? points[1][1] : mouseY;
		textSize(15);
		texto = (Math.pow(Math.pow(points[0][1]-vy,2)+Math.pow(points[0][0]-vx,2),.5)/coeCmToPx)+
		" ("+Math.abs(points[0][1]-vy)+", "+Math.abs(points[0][0]-vx)+") ";
		text(texto,mouseX+t,mouseY);
	}
	...
	void blocosCentrais(){
		...
		for (float line : layoutBlocosCentrais){
			...
			for (int i =0 ; i < line;i++){
				...
				py = espessuraAltar+(distBlocos[0]+vari*i)*tamQuadrado+(linha-1)*(tamCadeira+espacoEntreFileiras);
				rect(px,py,tamCadeira,tamCadeira);
				qtCadeiras++;
				// putChair(px,py,0); // isso trava
			}
			// direita
			for (int i =0 ; i < line;i++){
	...
	void blocosLaterais(){
		...
		varY = espessuraAltar+distBlocos[1]*tamQuadrado-tamCadeira*pow(2,.5)/2;
	...
	void liner(){
		...
		stroke(1);
		if (firstPressed){
			line(points[0][0]-paddingX,points[0][1]-paddingY,points[1][0]-paddingX,points[1][1]-paddingY);
		}else{
			line(points[0][0]-paddingX,points[0][1]-paddingY,mouseX-paddingX,mouseY-paddingY);
		}
	}
