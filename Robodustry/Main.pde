...
void mouseAutomatic() {
	...
	for (Construction c : constructions) {
      ...
      if (c.pointInsidePolygon(mouseX,mouseY)) {
				overConstruction = c;
				c.shined = true;
				overed = true;
			}
...
void mousePressed() {
	...
	if (!updateMenus(mouseButton == LEFT,mouseButton == RIGHT)) processPutting(mouseButton == LEFT);
...
void processPutting(boolean put) {
	if (onHandConstruction != null) {
		
		if (onHandConstruction.isTypeOfPipe) {
			onHandConstruction.px = (mouseX - onHandConstruction.tam / 2 - (mouseX - onHandConstruction.tam / 2 - transX) % (tamSquare / 2)) / zoom;
			onHandConstruction.py = (mouseY - onHandConstruction.tam / 2 - (mouseY - onHandConstruction.tam / 2 - transY) % (tamSquare / 2)) / zoom;
	    // onHandConstruction.px = mouseX - onHandConstruction.tam / 2;
			// onHandConstruction.py = mouseY  - onHandConstruction.tam / 2;
		} else{
		  onHandConstruction.px = (mouseX - (mouseX - transX) % (tamSquare / 2)) / zoom;
			onHandConstruction.py = (mouseY - (mouseY - transY) % (tamSquare / 2)) / zoom;
		}
		
		if (onHandConstruction.modusLinearis) {
			onHandConstruction.modusLinearis(mouseX - (mouseX + onHandConstruction.tam / 2 - transX) % (tamSquare / 2),mouseY - (mouseY + onHandConstruction.tam / 2 - transY) % (tamSquare / 2));
		}
		// else{
		// }
		// verify is can put
			...
			switch(cons.id) {
				...
				case "pipe":
					...
					switch(onHandConstruction.id) {
						...
						case "pipe":
						if (
							onHandConstruction.px > pa && onHandConstruction.px < pb && onHandConstruction.py > pc && onHandConstruction.py < pd || 
							onHandConstruction.px + onHandConstruction.tam > pa && onHandConstruction.px + onHandConstruction.tam < pb && onHandConstruction.py > pc && onHandConstruction.py < pd || 
							onHandConstruction.px + onHandConstruction.tam > pa && onHandConstruction.px + onHandConstruction.tam < pb && onHandConstruction.py + onHandConstruction.tam > pc && onHandConstruction.py + onHandConstruction.tam < pd || 
							onHandConstruction.px > pa && onHandConstruction.px < pb && onHandConstruction.py + onHandConstruction.tam > pc && onHandConstruction.py + onHandConstruction.tam < pd || 
							pow(pow(onHandConstruction.px + onHandConstruction.tam / 2 - pa - cons.tam / 2,2) + pow(onHandConstruction.py + onHandConstruction.tam / 2 - pc - cons.tam / 2,2),.5) < onHandConstruction.tam) {
							canPut = false;
							breakar = true;
						}
					break;
					// RC
					case "connector":
						if (pow(pow(onHandConstruction.px - pa,2) + pow(onHandConstruction.py - pc,2),.5) < onHandConstruction.r || 
							pow(pow(onHandConstruction.px - pb,2) + pow(onHandConstruction.py - pc,2),.5) < onHandConstruction.r || 
							pow(pow(onHandConstruction.px - pb,2) + pow(onHandConstruction.py - pd,2),.5) < onHandConstruction.r || 
							pow(pow(onHandConstruction.px - pa,2) + pow(onHandConstruction.py - pd,2),.5) < onHandConstruction.r || 
							pow(pow(onHandConstruction.px - pa - cons.tam / 2,2) + pow(onHandConstruction.py - pc - cons.tam / 2,2),.5) < onHandConstruction.r) {
							canPut = false;
							breakar = true;
						}
						break;
				...
				// C
				case "connector":
					pa = onHandConstruction.px;pb = onHandConstruction.px + onHandConstruction.tam;pc = onHandConstruction.py;pd = onHandConstruction.py + onHandConstruction.tam;
					switch(onHandConstruction.id) {
						// CR
						case "pipe":
						if (
					...
					// CC
					case "connector":
						if (pow(pow(onHandConstruction.px - cons.px,2) + pow(onHandConstruction.py - cons.py,2),.5) < onHandConstruction.r + cons.r) {
							canPut = false;
							breakar = true;
						}
						break;
			...
			if (onHandConstruction.id.equals(typeOfPipe)) {
				...
				for (Construction cons : constructions) {
					for (String typeOfConnector : typeOfConnectors) { 
						if (cons.id.equals(typeOfConnector)) {
							if (pow(pow(cons.px - onHandConstruction.px - onHandConstruction.tam / 2,2) + pow(cons.py - onHandConstruction.py - onHandConstruction.tam / 2,2),.5) == 0) {
								canPut = true;
								connectConstruction = cons;
								break;
							}
						}
					}
	...
	i++) {
		...
		switch(menu.name) {
			...
			case "Putting":
				...
				// up
				menu.addButton(new Button("var-onHandConstruction","rect","onHand",color(#A8FCCF),10,px + step + localX * (step + wBtUp),py + step,wBtUp,hBtUp,0));
				localX++;
				// menu.addButton(new Button("Level 2","rect","lvl2",color(#FFD640),px+step+localX*(step+wBtUp),py-hBtUp,wBtUp,hBtUp,0));
				break;
