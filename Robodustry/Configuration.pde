...
class Configuration {
  ...
  public int timeToDoubleClick = 300;
  public String idRunning = "peppa";
  Menu menu_putting = null;
  Button btOnHand = null, localMaterials = null, localOptions = null;
  ...
  public LinkedHashMap<String, Integer> timers = new LinkedHashMap<String, Integer>();
  ...
  Configuration() {
    idRunning = "idr." + hour() + ":" + minute() + ":" + second() + "*" + day() + ":" + month() + ":" + year();
    _loadMaterials();
    defineColors();
    defineMaterials();
  }
  ...
  private void _saveFabriquè() {
    try {
      String aux = "";
			ArrayList<Construction> puts = new ArrayList<Construction>();
			// coloca os macropieces primeiro c seus pieces primeiro
      for (Construction cons : constructions) {
				if (!cons.isTypeOfMacroPiece) continue;
				for (Construction p : ((MacroPiece)cons).pieces) {
					aux += p.getStringOfMe() + "\n";
					puts.add(p);
				}
        aux += cons.getStringOfMe() + "\n";
			...
			for (Construction cons : constructions) {
				...
				for (Construction p : puts) {
					if (cons.specificId.equals(p.specificId)) {
						hasPutting = true;
						break;
					}
				}
        if (!hasPutting) aux += cons.getStringOfMe() + "\n";
      }
  ...
  private ArrayList<Construction> splitterFabriquè(List<String> red) {
    ...
    try {
      for (String line : red) {
        numberLine ++;
        String[] parts = line.split(";"),aux;
				LinkedHashMap<Material, Float> mats;
        ...
        case"P":
          ...
          mats = new LinkedHashMap<Material, Float>();
          aux = parts[7].split("_");
          ...
          break;
				case"Mp":
          // Mp;Collector;collector;metal;32.0;320.0;320.0;
					// metal,50.0_aluminium,10.0;connector;ang;collector-17L-idr.17:3:59*6:7:2026-4261;---;false;false;false
         	mats = new LinkedHashMap<Material, Float>();
          aux = parts[7].split("_");
          for (int i = 0; i < aux.length; i++) {
            Material mat = this.getMaterial(aux[i].split(",")[0]).copy();
            if (mat != null) {
              ...
              println("ERRO ao processar construction:\n   material nao reconhecido: '" + aux[i] + "' in line " + numberLine);
					...
					for (String str : parts[8].split("_")){
						for (Construction cons2 : conss) {
							if (cons2.specificId.equals(str)) { // pega specificId
								pieces.add((Piece)cons2);
								break;
							}
						}
					}
          cons = new MacroPiece(parts[1], parts[2], Float.parseFloat(parts[4]), getMaterial(parts[3]), Float.parseFloat(parts[5]), Float.parseFloat(parts[6]), mats,pieces);
          // 8 inner
					println(((MacroPiece)cons).pieces.size());
					int gapMPConfigs = 2;
					((MacroPiece)cons).angle = Float.parseFloat(parts[9]);
					cons.definePointsConstruit();

					cons.specificId = parts[8+gapMPConfigs];
          cons.modusLinearis = parts[10+gapMPConfigs].equals("true");
          cons.effectiveModusLinearis = parts[11+gapMPConfigs].equals("true");
          cons.isTypeOfPipe = parts[12+gapMPConfigs].equals("true");
          // connects
          pos_connects.put(parts[8+gapMPConfigs], new ArrayList<String>());
          if (!parts[9+gapMPConfigs].equals("---")) {
            aux = parts[9+gapMPConfigs].split("_");
            for (String str : aux) {
              pos_connects.get(parts[8+gapMPConfigs]).add(str);
            }
          }
          break;
        default:
      ...
      println("ERRO ao processar construction:\n   " + e+" in line Cfg.splitterFabriquè()");
    }
    // put connects
    try {
            ...
            for (String str : pos_connects.get(key)) {
              for (Construction c2 : conss) {
                if (c2.specificId.equals(str)) {
                  ...
                  println(c1.specificId + " ->cfg " + c2.specificId);
                }
              }
            }
            if (c1.isTypeOfPipe) {
  ...
  // TIMER
  void resetTimer() {
    timers.put("init", millis());
  }
  void putTimer(String name) {
    timers.put(name, millis() - timers.get("init"));
  }
  // long calcTimer(String a,String b){
  // 	return timers.get(b)-timers.get(a);
  // }
  // RUNNING GAME SAVING
  ...
  private void _preProcessSaving(String type, Construction cons) {
    ...
    // tem q dar erro se n der pra colocar
      if (cons.isTypeOfMacroPiece) {
        for (Piece p : ((MacroPiece)cons).pieces) {
          if (Inv.removeMaterialsForConstruction(p)) constructions.add(p);
        }
    ...
    case "removeFromConstructions":
      ...
      // 	println(c.specificId);
      // 	for (Construction c1 : c.connections) {
      // 		println("   cfg>>" + c1.specificId);
      // 	}
      // }
      if (Inv.addMaterialsForConstruction(cons)) {
        ...
        if (cons.isTypeOfMacroPiece) {
          for (Construction p : ((MacroPiece)cons).pieces) {
            boolean inConstructions = false;
            for (Construction c : constructions) {
              if (c.specificId.equals(p.specificId)) {
                inConstructions = true;
                break;
              }
            }
            if (inConstructions) preProcessSaving("removeFromConstructions", p);
          }
        }
  ...
  private  Construction disformatter(String form) {
    ...
    for (int i = 0; i < aux.length; i++) {
      ...
      for (int j = 0; j < options.length; j++) {
        ...
        float qt = Float.parseFloat(options[j].split(",")[1]);
        if (mat  !=   null) {
          matsOp.put(mat, qt);	
          //  print(mat.id + " " + qt+"  ");
        } else {
        ...
        if (j ==  0) mats.put(mat, qt);
      }
      // println(" ");
      matsOps.add(matsOp);
      // coloca o primeiro material
    }
    //  type
    ...
    case  "P":
      ...
      // break;
    case "Mp":
      // println("Defining MacroPiece");
      ArrayList<Piece> pieces = new ArrayList<Piece>();
      String[] aux1 = formatter.get(8).split("_");
      for (String str : aux1) {
        for (Construction cons : prefabs) {
          if (cons.id.equals(str.split("-")[0])) { // pega o id, inves do specificID
            pieces.add((Piece)cons.copy());
            break;
          }
        }
      }
      Construction mp = new MacroPiece(name, id, tam, material, px, py, mats, pieces);
      for (Piece pc : pieces) { // coloca os macroPieces dos pieces,facilita depois
        pc.macroPiece = ((MacroPiece)mp);
      }
      mp.materialsOptions = matsOps;
      // println(mp.getClass().getSimpleName() + ">" + mp.specificId);
      return mp;
    default:
      println("ERRO ao processar prefab : \n   type nao reconhecido: '" + form + "' in  Cfg.disformatter()");
    }
    return  null;
  }

  // AUX
  boolean containsMatchMaterials(LinkedHashMap<Material, Float> mats, Material mat) {
    for (Material mat1 : mats.keySet()) {
      if (mat1.id.equals(mat.id)) return true;
    }
    return false;
  }
  void redefinePreFabs(Construction cons) {
    for (int i = 0; i < prefabs.size(); i++) {
      if (prefabs.get(i).id.equals(cons.id)) {
        prefabs.set(i, cons);
        break;
      }
    }
